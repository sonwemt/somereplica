...
function Comment({ comment, isLoggedIn, isReference = false }) {
  ...
  const handleRemoveAction = async () => {
    try {
      await deleteDoc(
        doc(db, "posts", `${comment.parentid}`, "comments", `${comment.id}`)
      );
      navigate(0);
    } catch (error) {
      console.log(error);
      ...
      <div className="commentContainer">
        ...
        <div className="comment-options">
          {isLoggedIn.displayName === comment.user ? (
            <div onClick={() => handleRemoveAction()} style={{ color: "gray" }}>
              remove
            </div>
          ) : null}
          <div style={{ color: "gray" }}>save</div>
          <div onClick={() => setShowReplyBox(showReplyBox ? false : true)}>
            reply
          </div>
          <TimeElapsed created={comment.localDate} />
          {isReference ? (
            <Link to={`/r/${comment.subreplica}/comments/${comment.parentid}`}>
              context
            </Link>
          ) : null}
        </div>
