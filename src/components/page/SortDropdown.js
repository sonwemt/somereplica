...
function SortDropdown({ sortFilter, setSortFilter }) {
  ...
  const updateSortString = (score, order) => {
    if (score) {
      ...
      order === "desc" ? setCurrentSorting("new") : setCurrentSorting("old");
    }
  };
      ...
      {showOptions ? (
        <div className="sort-options">
          <button
            ...
            onClick={() => handleSortChange(false, "desc")}
          >
            new
          </button>
          <button
            className="sort-by-old"
            onClick={() => handleSortChange(false, "asc")}
          >
            old
          </button>
          <button
            className="sort-by-upvotes"
            onClick={() => handleSortChange(true, "desc")}
          >
            top
          </button>
          <button
            className="sort-by-downvotes"
            onClick={() => handleSortChange(true, "asc")}
          >
            lowest score
          </button>
        </div>
